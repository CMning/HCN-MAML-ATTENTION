...
## Training options
flags.DEFINE_integer('pretrain_iterations', 0, 'number of pre-training iterations.')#预训练迭代次数
flags.DEFINE_integer('metatrain_iterations', 15000, 'number of metatraining iterations.') # 元学习迭代次数15k for omniglot, 50k for sinusoid
flags.DEFINE_integer('meta_batch_size', 25, 'number of tasks sampled per meta-update')#每个元更新采样的任务数
flags.DEFINE_float('meta_lr', 0.001, 'the base learning rate of the generator')#生成器的基本学习率
flags.DEFINE_integer('update_batch_size', 5, 'number of examples used for inner gradient update (K for K-shot learning).')#用于内部梯度更新的示例数（K表示K射击学习）
...
def main():
    ...
    model.summ_op = tf.summary.merge_all()#merge_all 可以将所有summary全部保存到磁盘，以便tensorboard显示。如果没有特殊要求，一般用这一句就可一显示训练时的各种信息了。
