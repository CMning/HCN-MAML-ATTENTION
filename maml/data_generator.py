...
class DataGenerator(object):
    ...
    def __init__(self, num_samples_per_class, batch_size, config={}):
        ...
        if FLAGS.datasource == 'sinusoid':#sinusoid正弦曲线
            #注意:这里只是方法的赋值，并没有调用方法，如果是调用应该这么写：self.generate = self.generate_sinusoid_batch()
            self.generate = self.generate_sinusoid_batch
            #amp_range=[0.1,5.0],phase_range=[0, np.pi],input_range=[-5.0, 5.0]。。。。类型都是list
            self.amp_range = config.get('amp_range', [0.1, 5.0])
            self.phase_range = config.get('phase_range', [0, np.pi])
            self.input_range = config.get('input_range', [-5.0, 5.0])
            self.dim_input = 1
            self.dim_output = 1
        elif 'omniglot' in FLAGS.datasource:#全方位
            self.num_classes = config.get('num_classes', FLAGS.num_classes)
            self.img_size = config.get('img_size', (28, 28))
            self.dim_input = np.prod(self.img_size)#用来计算所有元素的乘积，对于有多个维度的数组可以指定轴，如axis=1指定计算每一行的乘积。
            ...
            self.dim_input = np.prod(self.img_size)*3   #np.prod()函数用来计算所有元素的乘积
            self.dim_output = self.num_classes
    ...
    def make_data_tensor(self, train=True):
        ...
        for _ in range(num_total_batches):
            #sample(list, k)返回一个长度为k新列表，新列表存放list所产生k个随机不重复的元素,从folders里面随机取num_classes个数，组成一个新的list
            sampled_character_folders = random.sample(folders, self.num_classes)
            #random.shuffle随机排序列表
    ...
    def generate_sinusoid_batch(self, train=True, input_idx=None):
        ...
        amp = np.random.uniform(self.amp_range[0], self.amp_range[1], [self.batch_size])#功能：从一个均匀分布[low,high)中随机采样，注意定义域是左闭右开，即包含low，不包含high.
        phase = np.random.uniform(self.phase_range[0], self.phase_range[1], [self.batch_size])
        outputs = np.zeros([self.batch_size, self.num_samples_per_class, self.dim_output])#返回：返回来一个给定形状和类型的用0填充的数组；
        ...
        for func in range(self.batch_size):
            ...
            outputs[func] = amp[func] * np.sin(init_inputs[func]-phase[func])#np.sin(a) 对矩阵a中每个元素取正弦
